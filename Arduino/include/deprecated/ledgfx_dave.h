...
void DrawPixels(StripSection *position, float fPos, float count, CRGB color)
{
  auto leds = position->Data;

  // Calculate how much the first pixel will hold
  float availFirstPixel = 1.0f - (fPos - (long)(fPos));
  float amtFirstPixel = min(availFirstPixel, count);
  float remaining = min(count, position->Size - fPos);
  int iPos = fPos;
  ...
  if (remaining > 0.0f)
  {
    auto cFract = ColorFraction(color, amtFirstPixel);
    leds[iPos++] += cFract;
